...
def render():
  ...
  with st.sidebar.form("box and text color"):
    st.write("box and text Settings")
    COLOR = st.color_picker("Color picker", value='#FF4B4B', key=None, help=None, on_change=None, args=None, kwargs=None)
    ...
    LINETYPE = st.slider("Thickness", min_value=0, max_value=10, value=2)
  ...
  def display_image_recognition(image_path):
    ...
    for (top, right, bottom, left), member, min_dist  in zip(boxes, actors_list, dist_list):
        cv2.rectangle(img, (left, top), (right, bottom), (RED, GREEN, BLUE), 3)
